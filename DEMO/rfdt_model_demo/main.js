...
async function main() {
    ...
    const url = new URL(
        params.get("url") || "https://huggingface.co/datasets/ved1708/model_delay_3.5GHz/resolve/main/model.splat",
        location.href,
    );
    ...
    const frame = (now) => {
        ...
        if (carousel) {
            ...
            // Slowly pan the camera angle to cover the room
            inv = rotate4(inv, 0.2 * Math.sin(t), 0, 1, 0);
            inv = rotate4(inv, 0.1 * Math.cos(t * 0.5), 1, 0, 0);

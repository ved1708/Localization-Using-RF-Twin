...
def generate_ideal_dataset(scene, rx_locs, tx_loc, output_dir, spectrum_type='mpc', spec_min=None, spec_max=None, phase_norm_params=None, rx_yaw=None):
    ...
    if spectrum_type != 'phase':
        if spec_max is not None and spec_min is not None:
            print(f"Using CLI provided stats (Min/Max dB): {spec_min}, {spec_max}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rx-pos", type=float, nargs=3, default=None,
    ...
    parser.add_argument("--spec-max", type=float, default=None, help="Normalization max (dB) for non-phase")
    parser.add_argument("--rg-scale", type=float, default=None, help="RG scale for phase normalization")
    parser.add_argument("--b-lo-db", type=float, default=None, help="B min (dB) for phase normalization")
    parser.add_argument("--b-hi-db", type=float, default=None, help="B max (dB) for phase normalization")
    ...
    elif args.rx_pos_file is not None:
        rx_locs = []
        with open(args.rx_pos_file, 'r') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) >= 3:
                    rx_locs.append([float(parts[0]), float(parts[1]), float(parts[2])])
        print(f"Loaded {len(rx_locs)} RX positions from {args.rx_pos_file}")
    else:
